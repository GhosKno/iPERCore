...
from iPERCore.services.preprocess import human_estimate


if __name__ == "__main__":
    from iPERCore.services.options.options_inference import InferenceOptions
    origin_videos = glob.glob('/data/jupyter/dy_dance_video_partial_frames/query/*/*_frames')
    origin_videos = origin_videos + glob.glob('/data/jupyter/dy_dance_video_partial_frames/gallery/*/*_frames')
    i = 0
    step = 256
    ...
    while i < len(origin_videos):
        ref_paths = []
        for j in range(step):
            if i >= len(origin_videos): break
            label = origin_videos[i].split('/')[-2]
            video_id = os.path.basename(origin_videos[i])[:-4]
            ref_paths.append('path?={},name?={},pose_fc?=300'.format(origin_videos[i], video_id+'#{}#'.format(label)))
            i+=1
